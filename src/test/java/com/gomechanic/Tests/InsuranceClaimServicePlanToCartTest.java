...
import com.gomechanic.pages.InsuranceClaimPage;
...
public class InsuranceClaimServicePlanToCartTest extends BaseTestSuite{

	@Test(description="Validate user is able to select 'Doorstep Accidental Inspection' from 'Insurance Claim Services'")
	public void selectTyresAndWheelCarePlan(Method method) throws InterruptedException {
		...
		detailsPage.selectRequiredFuel(data1[0][3]);
			...
			Thread.sleep(8000);
		   	String[][] data2 = excelData.readData("InsuranceClaim");
		   	InsuranceClaimPage insuranceClaimPage=new InsuranceClaimPage(driver, testLog);
		   	insuranceClaimPage.selectInsuranceClaimService();
			Assert.assertEquals(data2[0][0], insuranceClaimPage.verifyInsuranceClaimServiceMenuBarText());
			testLog.log(LogStatus.INFO, "Expected Insurance Claim Service Menu Tab Text: '" + data2[0][0] + "' is verified with Actual: '"
					+ insuranceClaimPage.verifyInsuranceClaimServiceMenuBarText() + "'");
			Assert.assertEquals(data2[0][1], insuranceClaimPage.verifyHeaderText());
			testLog.log(LogStatus.INFO, "Expected 'Accidental Repairs ' Header Text from 'Insurance Claim Service': '" + data2[0][1] + "' is verified with Actual: '"
					+ insuranceClaimPage.verifyHeaderText() + "'");
			Assert.assertEquals(data2[0][2], insuranceClaimPage.verifyServiceNameText());
			testLog.log(LogStatus.INFO, "Expected 'Doorstep Accidental Inspection' Service Name 'Insurance Claim Service': '" + data2[0][2] + "' is verified with Actual: '"
					+ insuranceClaimPage.verifyServiceNameText() + "'");
			insuranceClaimPage.clickAddToCart();
			Thread.sleep(3000);
