...
import com.gomechanic.pages.CustomServicePage;
...
public class AddCustomServicesPlanToCartTest extends BaseTestSuite{

	@Test(description="Validate user is able to select 'Engine Scanning' from 'Custom Services'")
	public void selectAmaronPlan(Method method) throws InterruptedException {
		...
		detailsPage.selectRequiredFuel(data1[0][3]);
			...
			Thread.sleep(8000);
			String[][] data2 = excelData.readData("CustomServices");
			CustomServicePage customServicePage=new CustomServicePage(driver, testLog);
			customServicePage.clickScrollMenuBarRight();
			customServicePage.selectCustomService();
			Assert.assertEquals(data2[0][0], customServicePage.verifyCustomServiceMenuBarText());
			testLog.log(LogStatus.INFO, "Expected Custom Service Menu Tab Text: '" + data2[0][0] + "' is verified with Actual: '"
					+ customServicePage.verifyCustomServiceMenuBarText() + "'");
			Assert.assertEquals(data2[0][1], customServicePage.verifyHeaderText());
			testLog.log(LogStatus.INFO, "Expected 'Inspection' Header Text from 'Custom Service': '" + data2[0][1] + "' is verified with Actual: '"
					+ customServicePage.verifyHeaderText() + "'");
			Assert.assertEquals(data2[0][2], customServicePage.verifyServiceNameText());
			testLog.log(LogStatus.INFO, "Expected 'Engine Scanning' Service Name 'Custom Service': '" + data2[0][2] + "' is verified with Actual: '"
					+ customServicePage.verifyServiceNameText() + "'");
			customServicePage.clickAddToCart();
			Thread.sleep(3000);
